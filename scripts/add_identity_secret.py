...
ACCOUNTS_JSON_PATH = PROJECT_ROOT / "data" / "accounts.json"
...
def build_mafile_indexes(mafiles_dir: Path) -> Tuple[Dict[str, str], Dict[str, str], Dict[str, str]]:
    """
    ...
    for path in mafiles_dir.iterdir():
        ...
        account_name = data.get("account_name") or data.get("accountName") or data.get("login")
...
def find_identity_secret(
    login: Optional[str], steam_id: Optional[int],
    idx_steamid: Dict[str, str], idx_account: Dict[str, str], idx_basename: Dict[str, str]
) -> Optional[str]:
...
def update_accounts(accounts_path: Path, target: Optional[str] = None) -> int:
    ...
    for key, account in accounts.items():
        ...
        identity_secret = find_identity_secret(login, steam_id, idx_steamid, idx_account, idx_basename)
        if identity_secret:
