...
class PluginManager:
    ...
    def load_enabled(self, enabled_names: List[str], app_context: Dict[str, object]) -> None:
    ...
    def _safe_load(self, name: str, app_context: Dict[str, object]) -> None:
        ...
        try:
            ...
            if plugin_obj is None or not hasattr(plugin_obj, "setup"):
                self._logger.error("Плагин '%s' не экспортирует объект 'plugin' совместимый с PluginBase", name)
                return
